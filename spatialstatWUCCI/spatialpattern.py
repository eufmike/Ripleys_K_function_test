...
def spestedge(input_array_ref, input_array_all, rstart, rend, 
    ...
    '''
    countlist, RList, pointcountref, pointcountall = countlistgeneratoredge(input_array_ref, 
                        rstart, rend, 
                        density, rsize, rstep, 
                        downsize, pointsizelimit,
                        seed)
...
# ----------------------------------------------------------------------------
def edgecorretion(input_array):
    return
    
# ----------------------------------------------------------------------------

def countlistgeneratoredge(input_array, rstart, rend, 
                        density, rsize, rstep, 
                        downsize, pointsizelimit,
                        seed):
    # ----------------------------------------------------------------------------
    ...
    # get counts from input_array
    pointcountref = input_array.shape[0]
    ...
    if pointcountref > pointsizelimit:
        if downsize == True:
            random.seed(seed)
            downsize_idx = random.sample(list(range(pointcountref)), pointsizelimit)
            input_array = input_array[downsize_idx]
            pointcountref = pointsizelimit
    ...
    # ----------------------------------------------------------------------------
    # add index to the input array
    input_array = arrayaddidx(input_array) 

    for i in tqdm.trange(pointcountref):
        # assign ref point
        refxy = input_array[i, 1:]
        refidx = int(input_array[i, 0])
